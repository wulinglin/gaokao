...
#pwd可发现当前目录在web下，所以写的方法如下：(另，在这里打开的terminal是linux系统）
# try:
#     mysql = pymysql.connect('172.19.235.29', 'root', 'mingming')
...
def recommend_getInfo(request,html):
    for key in ['province','point','yingjie','hukou','sex','classy', 'nation','add_point']:
        temp=request.GET.get(key,'')
        request.session[key]=temp#mbti在之后
    return render(request,html)#django1.11居然没有render_to_response()

def recommend_junior(request):#专科推荐
    province = request.session['province'];nation = request.session['nation'];
    if province in ['上海', '浙江', '海南', '山东']:#新高考没有专科：第三批=专科？那就和下面本科一起推荐
        response = '<p>上海、浙江已实行新高考，暂不提供上海、浙江的推荐。</p>'
        return HttpResponse(response)
    elif province == '西藏':
        if nation == '是':batches17 = ['专科（少数民族）']
        else:batches17 = ['专科（汉族）']
        batches_all = ['普通本科', '第三批']
...
def recommend_school(request):
    ...
    if (request.GET.get('add_point')=='' and request.GET.get('point')=='') or want_province ==[]:
       ...
       #以下所有batches都是本科。
        # 上海、浙江、海南、山东，合并本科。实行新高考，暂不推荐。浙江分了一二三段，其它都是本科
        #本科 自主招生 艺术类本科 体育类本科 国家专项计划
    ...
    #西藏：重点本科：少数民族325分，汉族440分。普通本科：少数民族285分，汉族355分。专科(高职)：少数民族245分，汉族315分。
        if province=='西藏':#！不知道怎么推荐，因为学校-省份分数只有第一批、第二批、第三批，不知道是少数民族还是汉族
    ...
    #其它：依旧实行一二三批，但是有的只有一二批。
            ...
            line17_sql= nation_line.objects.filter(province=province, year=2017,#datetime.datetime.now().year,
            ...
            if sub_point < 0:  # 如果该用户的分数没有达到二本分数线，暂不推荐。
                line17_sql = nation_line.objects.filter(province=province, year=datetime.datetime.now().year,classy=classy,batch__icontains='专科')
...
def major_cat(request):
    ...
    major16_lb_t=major16_lb.objects.filter(mbti__icontains=str(mbti_answer))
    ...
    mbti_explanation=mbti_chractor.objects.values('explanation').filter(mbti=mbti_answer)
...
def major_maj(request):
    ...
    if len(lingyu)==0:
        ...
        major16_lb_t = major16_lb.objects.filter(mbti__icontains=str(mbti_answer),catalogue__in=lingyu)
...
def recommend_isMajor(request):
    ...
    if (request.GET.get('add_point') == '' and request.GET.get('point') == '') or want_province == []:
        ...
        major16_lb_t = major16_lb.objects.all()
        major16_lb_df=pd.DataFrame.from_records(major16_lb_t.values())
        print('major16_lb_df',major16_lb_df)
        lb_dict = {}
...
def recommend_res(request):
    ...
    if len(catalogue)<1 and len(want_major)<1:
        ...
        if province in ['上海', '浙江', '海南', '山东']:
            ...
            print(line17, user_point)
            if sub_point < 0:  # 如果该用户的分数没有达到二本分数线，暂不推荐。
                line17_sql = nation_line.objects.filter(province=province, year=datetime.datetime.now().year,
                ...
                if len(catalogue)>0 and ''.join(want_major)=='':
                    univer_major_line_lb_t=univer_major_line_lb.objects.filter(province=province,school__in=candidate_school_1,
                    ...
                    candidate_school_t = univer_info.objects.filter(school__in=[key[0] for key in candidate_school_catalogue.keys()])
    ...
    return render_to_response('recommend_isMajor.html', {'error': error})
            ...
            # bwc_dict = {}
            # b, w, c = [], [], []
            # for k, j, v in zip([each[0] for each in candidate_school_list], [each[1] for each in candidate_school_list],
            #                    labels_bwc):
            #     if v == '保': b.append((k, j))
            #     if v == '稳': w.append((k, j))
            #     if v == '冲': c.append((k, j))
...
# print(sorted(candidate_school_dict.items(), key=lambda d: d[1], reverse=True))
# candidate_school_sort = sorted(candidate_school_dict.items(), key=lambda d: d[1],
#                                reverse=True)  # sorted以后是list嵌套元组
# candidate_school = [i[0] for i in candidate_school_sort[:10]]
# return render(request, 'recom_sch_res.html', {'candidate_school': candidate_school})

# school_dict = {'development': {0: 0, 1: 1, 2: 2}, 'is_doctor': {0: 0, 1: 1},
#                'is_master': {0: 0, 1: 1},
#                'leixing_int': {0: 0, 1: 3, 2: 6}, 'provincial_capital': {0: 0, 1: 1},};

# def recommend_MajorSchool(request):
#     catalogue=request.GET.getlist('catalogue')
#     want_major=request.GET.getlist('want_major')
#     request.session['catalogue']=catalogue;request.session['want_major']
#     if len(catalogue)>0 and len(want_major)==0:
#         
#     if len(want_major) > 0 and len(catalogue) == 0:
#
#     else:
#         # raise Http404()
#         return recommend_isMajor(request)
  ...
  # lb_dict={'文学': ['艺术类', '中国语言文学类', '新闻传播学类', '外国语言文学类', '设计类', '轻工纺织食品类', '历史学类', '交通运输类', '工商管理类', '法学类', '电气信息类'],
  #    '历史学': ['历史学类', '外国语言文学类'],
  #    '工学': ['农业工程类', '机械类', '海洋工程类', '电子信息科学类', '电气信息类', '自动化类 ', '自动化类', '新闻传播学类', '武器类', '土建类', '水利类', '设计类',
  #           '轻工纺织食品类', '能源动力学', '能源动力类', '临床医学与医学技术类', '交通运输类', '机械类 ', '环境与安全类', '环境生态类', '环境科学类', '海洋科学类', '公安技术类',
  #           '地矿类', '测绘类', '材料类', '职业技术教育类', '仪器仪表类', '生物科学类', '生物工程类', '林业工程类', '化学类', '化工与制药类', '核工程类', '航空航天类',
  #           '管理科学与工程类', '工程力学类', '地质学类'], '法学': ['政治学类', '社会学类', '公安学类', '法学类'],
  #    '教育学': ['职业技术教育类', '中国语言文学类', '体育学类', '临床医学与医学技术类', '教育学类', '职业技术教育', '外国语言文学类'],
  #    '管理学': ['工商管理类', '公安技术类', '体育学类', '水利类', '轻工纺织食品类', '农业经济管理类', '旅游类', '经济学类', '交通运输类', '机械类', '管理科学与工程类', '公共管理类',
  #            '公安学类', '法学类'], '农学': ['植物生产类', '水产类', '生物科学类', '森林资源类', '环境与安全类', '环境生态类', '动物医学类', '动物生产类', '草业科学类'],
  #    '军事学': ['政治学类', '武器类', '土建类', '航空航天类', '公安学类', '公安技术类', '电气信息类', '测绘类', '材料类'],
  #    '经济学': ['管理科学与工程类', '统计学类', '能源动力类', '经济学类'],
  #    '理学': ['电子信息科学类', '数学类', '物理学类', '地球物理学类', '材料科学类', '心理学类', '统计学类', '生物科学类', '力学类', '环境科学类', '化学类', '海洋科学类',
  #           '电气信息类', '地质学类', '地理科学类', '大气科学类'], '哲学': ['哲学类'],
  #    '医学': ['预防医学类', '药学类', '生物科学类', '临床医学与医学技术类', '口腔医学类', '基础医学类', '化工与制药类', '法医学类', '法学类', '动物医学类', '电气信息类', '护理学类']}


# bwc_dict = {}
# b,w,c=[],[],[]
# for k,v in zip(candidate_school_dict.items(), labels_bwc):
#     if v=='保':
#         b.append((k[0],k[1]))
#     if v=='稳':
#         w.append((k[0],k[1]))
#     if v == '冲':
#         c.append((k[0], k[1]))
# bwc_dict['保'] = b;bwc_dict['稳'] =w;bwc_dict['冲'] =c#{'a': ('保', 0.544),...}
# print('bwc_dict\n',bwc_dict)#{'冲': [('沈阳工业大学', 0.501), ...}


# bwc_dict = {}
# for k,v in zip(candidate_school_dict.items(), labels_bwc):
#     bwc_dict[k[0]]=v,k[1]
# bwc_dict = dict(sorted(bwc_dict.items(), key=lambda x: x[1][1],reverse=True))# [('a', ('保', 0.544)), ...]
# # key=lambda x: x[1][1]可以对{'a': ('保', 0.544),...}的0.544排序
# bwc_df = pd.DataFrame(bwc_dict).T
# # bwc_res=bwc_df[bwc_df[0] == '保']
# bwc_res=bwc_df.to_dict(orient='split')['data']
# bwc_json = json.dumps(bwc_dict, ensure_ascii=False)  # {"保": [["安徽工业大学", 0.1], ...]}


# print(bwc_res)
# bwc_json = json.dumps(bwc_res, ensure_ascii=False)
# print(bwc_json)

# return HttpResponse(bwc_json, content_type='application/json;charset=utf8')
# data['group_sort'] = data['comment_num'].groupby(data['cate']).rank(ascending=0, method='dense')

# print(bwc_series)
# bwc_res = {'稳': list(bwc_series[bwc_series == '稳'].index), '保': list(bwc_series[bwc_series == '保'].index),
#            '冲': list(bwc_series[bwc_series == '冲'].index), }

# print(sorted(candidate_school_dict.items(),key=lambda d:d[1],reverse=True))
# candidate_school_sort=sorted(candidate_school_dict.items(),key=lambda d:d[1],reverse=True)#sorted以后是list嵌套元组
# candidate_school=[i[0]  for i in candidate_school_sort[:10]]
